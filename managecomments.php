...
$result = $conn->query("
    SELECT comments.*, users.username ,blogs.title
    FROM comments 
...
<table>
    ...
    while ($row = $result->fetch_assoc()) { ?>
        ...
        <td><?php echo $row['title']; ?></td>
            ...
            <td>
            <?php if ($row['status'] != 1): ?>
                        <a href="approve_comment.php?id=<?php echo $row['id']; ?>" class="approve">Approve</a>
                    <?php endif; ?>

                    <?php if ($row['status'] !=0): ?>
                        <a href="reject_comment.php?id=<?php echo $row['id']; ?>" class="reject">Reject</a>
                    <?php endif; ?>            </td>
        </tr>
    <?php } ?>
</table>
